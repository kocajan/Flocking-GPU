/**
 * \file BoidPopulationRegulationUtils.cpp
 * \author Jan Koča
 * \date 01-05-2026
 * \brief Implementation of boid spawning, deletion, and population regulation.
 */
...
// -----------------------------------------------------------------------------
//  Boid creation / deletion helpers
// -----------------------------------------------------------------------------
...
void spawnBoid(SimState& s, BoidType type, const Vec3& pos, const Vec3& vel) {
    ...
    if (type == BoidType::Basic) {
        ...
        std::printf(
            "Warning: Unknown boid type in spawnBoid(). (type=%d)\n",
            static_cast<int>(type)
        );
    }
}

static void removeLastBoid(Boids& b, BoidType type) {
    if (type == BoidType::Basic) {
        b.posBasic.pop_back();
        b.velBasic.pop_back();
        b.accBasic.pop_back();
        ...
        std::printf(
            "Warning: Unknown boid type in removeLastBoid(). (type=%d)\n",
            static_cast<int>(type)
        );
    }
}
...
// -----------------------------------------------------------------------------
//  Slot-swap deletion helpers
// -----------------------------------------------------------------------------

static void moveLastBoidIntoSlot(SimState& s, BoidType type, int slotIdx) {
    Boids& b = s.boids;
    ...
    if (type == BoidType::Basic) {
        ...
        std::printf(
            "Warning: Unknown boid type in moveLastBoidIntoSlot(). (type=%d)\n",
            static_cast<int>(type)
        );
    }
}
...
void spawnBoids(SimState& s, BoidType type, int& count,
                int howMany, float x, float y, float z) {
...
int deleteBoidsInRadius(SimState& s, BoidType type, int& count,
    ...
    if (type == BoidType::Basic)
        ...
        std::printf(
            "Warning: Unknown boid type in deleteBoidsInRadius(). (type=%d)\n",
            static_cast<int>(type)
        );
        return 0;
    }
...
void regulateBoidPopulation(SimState& s, BoidType type,
                            int& count, int target) {

    const int delta = target - count;
    const int maxDelta =
        static_cast<int>(s.maxBoidPopulationChangeRate.number());
